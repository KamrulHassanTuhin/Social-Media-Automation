...
export function Dashboard() {
  ...
  const createItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const projectName = String(form.get("project") || "Content Studio");
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <div className="workspace-switcher"><div className="workspace-avatar">NS</div><div className="workspace-copy"><strong>Content Studio</strong><span>{useApi ? `${members.filter((member) => member.status === "ACTIVE").length} active members` : "Internal workspace"}</span></div><ChevronDown size={15} className="muted-icon" /></div>
...
function CreateModal({ projects, onClose, onSubmit }: { projects: WorkspaceProject[]; onClose: () => void; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  const options = projects.length ? projects : [{ id: "project_demo", name: "Content Studio" }, { id: "project_growth", name: "Growth Lab" }];
